...
namespace VRCTranscriptMod.VRCTranscribe {
    class TranscriptPlayerUi {
        ...
        private static void onQuickMenuOpen(VRC.Core.APIUser param_1) {
            ...
            transcriptToggle.State = TranscriptPlayerOverrides.IsWhitelisted(currently_active_uid);
        }
        ...
        public static void Init() { 
            ...
            transcriptToggle = new ToggleButton(
                ...
                new Action<bool>((state) => {
                    TranscriptPlayerOverrides.SetOverride(currently_active_uid, state);
                }),
            ...
            foreach(MethodInfo method in typeof(MenuController).GetMethods().Where(mi => mi.Name.StartsWith("Method_Public_Void_APIUser_") && !mi.Name.Contains("_PDM_")))
                VRCTranscriptMod.Instance.HarmonyInstance.Patch(method, postfix: new HarmonyMethod(typeof(TranscriptPlayerUi).GetMethod(nameof(onQuickMenuOpen), BindingFlags.NonPublic | BindingFlags.Static)));
        }
